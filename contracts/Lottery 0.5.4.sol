...
contract Lottery {
    ...
    function getTeam1() public view returns (address payable[] memory) {
     return team1;
        }

        
        
    function getTeam2() public view returns (address payable[] memory) {
     return team2;
        }
    
    function random() private returns (uint) {

      uint random1 = uint(keccak256(abi.encodePacked(now, nonce, manager))) % 100;
      nonce++;
      randd = random1;
      return randd;
    ...
    function pickWinner() public restricted returns (uint) {
        ...
        if (winteam == 0) {
              uint winamount = address(this).balance/team1.length;
              winningamount = winamount;
              for (uint i = 0;i<team1.length;i++) {
                 address payable a = team1[i];
                 a.transfer(winamount);
           
                  
              }
               team1 = new address payable[](0);
                   team2 = new address payable[](0);
        
        k = 1;


        } else {
            uint winamount1 = address(this).balance/team2.length;
